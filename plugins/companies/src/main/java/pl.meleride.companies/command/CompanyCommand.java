...
public class CompanyCommand {
  ...
  @CommandInfo(name = "company", userOnly = true, permission = "meleride.companies.companies")
  public void companyCommnad(CommandSender commandSender, CommandContext context) {
    switch (context.getParam(0)) {
      case "create": new CompanyCreateCommand(this.plugin).execute(commandSender, context);
      case "list": new CompanyListCommand(this.plugin).execute(commandSender, context);
    }
  }
