...
public class EconPlayer {
  ...
  private static Map<UUID, EconPlayer> econPlayerMap = new HashMap<>();
  ...
  public boolean exchange(Currency from, Currency to, double amount) {
    if(this.pocketBalance.get(from) == null ||
            this.pocketBalance.get(from) == 0.0) return false;
    if(amount <= 0 || amount > this.pocketBalance.get(from)) return false;
  ...
  public boolean charge(Currency currency, double amount) {
    if(!has(currency, amount)) return false;
